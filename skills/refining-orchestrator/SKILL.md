...
description: Orchestrates the three checkpoint-based question stages of the refining stage. Dispatches analysis-agent (checkpoints 1–2) and proposal-agent (checkpoint 3) to drive code-grounded Q&A, divergence detection, and proposal generation. Progress tracked in status.json via question_checkpoint (0→1→2→3). Despite the legacy term "brainstorming" used in earlier docs, this skill performs structured convergence, not divergent ideation.
---
...
|-----------------------|-------------------|-----------------------------|------|----------------------------|
| `0` or absent | `analysis-agent` | Phase 1 — read code + prompt, ask prompt-input questions | Broad code-grounded questions (≥ 3) | `question_checkpoint: 1` |
| `1` | `analysis-agent` | Phase 2 — analyze Phase 1 answers, ask follow-up questions | Divergence detection + follow-ups (≥ 3) | `question_checkpoint: 2`, `prompt.md` rewritten |
| `2` | `proposal-agent` | Step 1 — read refined `prompt.md`, deep code exploration | Code-first gap analysis, proposal-transition questions, generate `proposal.md` + phase plans | `question_checkpoint: 3`, stage → `proposing` |
| `3` | (none) | — | All checkpoints complete; skip dispatch | unchanged |

**Hard gate**: stage CANNOT advance from `refining` to `proposing` until `question_checkpoint === 3`.
...
| Agent | Activity | Max tool calls | Subagent allowed? |
|-------|----------|----------------|-------------------|
| `analysis-agent` | Phase 1 (CP 0→1) code reading | 15 | No |
| `analysis-agent` | Phase 2 (CP 1→2) targeted re-reads | 5 | No |
| `proposal-agent` | Direct code-reading | 10 | Yes (`explore` subagents allowed and encouraged) |
...
#### Task prompt template (analysis-agent — used when `question_checkpoint` is 0 or 1)
...
- question_checkpoint: {0 or 1}
- Tool budget — Phase 1: 15 code-reading calls; Phase 2: 5 targeted re-reads. Subagents are forbidden.

## Project Context
{content of .harness-task/context.md, or "(not provided)"}

## Instruction
Execute your full workflow as defined in your agent definition (`agents/analysis-agent.md`). Persist `question_checkpoint` to `status.json` after every phase. Do not advance the stage; only update `question_checkpoint`.
```

#### Task prompt template (proposal-agent — used when `question_checkpoint` is 2)
...
## Instruction
Execute your full workflow as defined in your agent definition (`agents/proposal-agent.md`). After producing artifacts, set `question_checkpoint: 3` and `stage: proposing` in `status.json`, populate `phases[]`, set `current_phase` to the first phase id, then stop and return control to the dev skill. Do NOT advance the stage past `proposing` — user confirmation is owned by the dev skill.
```
...
|-------------------------------|-------|--------------------|-----------|
| `1` | `analysis-agent` | `status.json.question_checkpoint === 1` | (no script gate at this checkpoint — fast-path back to Step 1) |
| `2` | `analysis-agent` | `prompt.md` rewritten with refined requirements; `status.json.question_checkpoint === 2` | (no script gate at this checkpoint) |
| `3` | `proposal-agent` | `prompt.md` finalized; `proposal.md` generated; `phases/PH-*.md` generated; `status.json.question_checkpoint === 3` and `stage === "proposing"` | **Run `hooks/validate-artifacts --change-dir <path> --gate refining-to-proposing`. If non-zero, surface the error to the user verbatim and STOP — do not silently advance.** |
...
- If `question_checkpoint === 3` and `stage === "proposing"`: orchestration complete. Return control to `harness-task:dev` for the user-confirmation flow that owns `proposing → executing`.
- If `question_checkpoint < 3`: the agent was interrupted mid-flow. The next invocation of this skill resumes per the **Dispatch Table**.
...
| `status.json` has `question_checkpoint` outside `{0,1,2,3}` or non-integer | Step 1 schema check | Stop. Surface the malformed value. Ask the user whether to reset (manually) or recover. |
| `validate-artifacts` exits non-zero (CP3 only) | Step 4 | Forward the script's stderr JSON to the user. Do NOT advance the stage. The agent or user must fix the artifacts; on re-invocation, the gate is re-run. |
...
| Partial artifacts from a previous failed CP3 (e.g., `proposal.md` exists but `phases/` is empty) | Step 4 gate | The gate script will fail; treat as a normal gate failure and surface. Do NOT delete partial artifacts automatically. |
...
|----------|------------|----------|
| Updated `prompt.md` | `analysis-agent` (Phase 2 end) and `proposal-agent` (Step 6) | `.dev-changes/{safe-branch-dir}/prompt.md` |
...
| `status.json` updates | Both agents (only the fields each owns) | `.dev-changes/{safe-branch-dir}/status.json` |
...
4. **Run the gate script at CP3** — `hooks/validate-artifacts --gate refining-to-proposing` is mandatory before returning control to `dev`.
5. **Gate on `question_checkpoint === 3`** — the stage CANNOT advance from `refining` to `proposing` until this is true.
6. **`prompt.md` is the single source of truth** for downstream agents.
7. **`proposal.md` and `phases/PH-*.md` are mandatory artifacts** at CP3 completion.
